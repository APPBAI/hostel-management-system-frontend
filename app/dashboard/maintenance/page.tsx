...
const maintenanceEntries = [
  { id: "MNT-001", resident: "Amara Osei", room: "A101", issue: "Broken light fixture", priority: "Medium", status: "In Progress", date: "2025-04-30" },
  { id: "MNT-002", resident: "Kwame Boateng", room: "B204", issue: "Leaking faucet", priority: "High", status: "Pending", date: "2025-04-29" },
  { id: "MNT-003", resident: "Abena Mansa", room: "C301", issue: "Door lock issue", priority: "Critical", status: "Active", date: "2025-04-30" },
  { id: "MNT-004", resident: "Kofi Mensah", room: "A101", issue: "AC not cooling", priority: "Low", status: "Completed", date: "2025-04-25" },
  { id: "MNT-005", resident: "Efua Appiah", room: "B102", issue: "Internet port broken", priority: "Medium", status: "In Progress", date: "2025-04-28" },
];
    ...
    <div className="flex flex-col space-y-8 py-4 pb-20">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div className="space-y-1">
          <h1 className="text-3xl font-black tracking-tight text-gray-900 leading-tight">Maintenance</h1>
          <p className="text-[11px] text-gray-400 font-extrabold uppercase tracking-[0.2em]">Manage and track facility repairs</p>
        </div>
      ...
      <div className="space-y-6 pt-4">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div className="space-y-1">
            ...
            <p className="text-[11px] text-gray-400 font-extrabold uppercase tracking-widest">Active & Recent tickets</p>
          </div>
        ...
        <div className="border border-gray-100 rounded-2xl bg-white overflow-hidden shadow-sm">
          <Table>
            <TableHeader className="bg-gray-50/50">
              <TableRow className="border-gray-100 hover:bg-transparent">
                <TableHead className="text-[10px] font-black text-gray-400 uppercase tracking-widest px-6 h-12">Ticket ID</TableHead>
                <TableHead className="text-[10px] font-black text-gray-400 uppercase tracking-widest px-6 h-12">Resident / Room</TableHead>
                <TableHead className="text-[10px] font-black text-gray-400 uppercase tracking-widest px-6 h-12">Issue Description</TableHead>
                <TableHead className="text-[10px] font-black text-gray-400 uppercase tracking-widest px-6 h-12">Priority</TableHead>
            ...
            <TableBody>
              {maintenanceEntries.map((entry) => (
                <TableRow key={entry.id} className="border-gray-50 hover:bg-gray-50/30 transition-colors">
                  <TableCell className="px-6 py-4 text-xs font-bold text-gray-900">{entry.id}</TableCell>
                  <TableCell className="px-6 py-4">
                    <div className="flex flex-col">
                      <span className="text-xs font-bold text-gray-900">{entry.resident}</span>
                      <span className="text-[9px] font-extrabold text-gray-400 uppercase tracking-wider">Room {entry.room}</span>
                    </div>
                  </TableCell>
                  <TableCell className="px-6 py-4 text-xs font-medium text-gray-500">{entry.issue}</TableCell>
                  <TableCell className="px-6 py-4">
                    <div className="flex items-center gap-2">
                      <div className={`size-1.5 rounded-full ${
                        entry.priority === 'Critical' ? 'bg-red-600' : 
                        entry.priority === 'High' ? 'bg-orange-500' : 
                        entry.priority === 'Medium' ? 'bg-blue-500' : 'bg-gray-300'
                      }`} />
                      <span className="text-xs font-bold text-gray-900">{entry.priority}</span>
                    </div>
                  </TableCell>
                  <TableCell className="px-6 py-4">
                    <Badge variant="outline" className={`rounded-md px-2 py-0.5 text-[9px] font-black uppercase tracking-widest border-none ${
                      entry.status === 'Active' ? 'bg-red-50 text-red-600' : 
                      entry.status === 'In Progress' ? 'bg-blue-50 text-blue-600' : 
                      entry.status === 'Pending' ? 'bg-yellow-50 text-yellow-600' : 'bg-green-50 text-green-600'
                    }`}>
                      {entry.status}
                    </Badge>
                  </TableCell>
                  <TableCell className="px-6 py-4">
                    <DropdownMenu>
                      ...
                      <DropdownMenuContent align="end" className="rounded-xl border-gray-100">
                        <DropdownMenuItem className="text-xs font-bold rounded-lg">Update Status</DropdownMenuItem>
                        <DropdownMenuItem className="text-xs font-bold rounded-lg">Assign Staff</DropdownMenuItem>
                        <DropdownMenuItem className="text-xs font-bold rounded-lg text-red-600">Close Ticket</DropdownMenuItem>
                      </DropdownMenuContent>
                    </DropdownMenu>
                  </TableCell>
                </TableRow>
          ...
          <div className="px-6 py-4 border-t border-gray-50 bg-white flex items-center justify-between">
            <span className="text-[10px] font-bold text-gray-400">Showing 5 of 156 tickets.</span>
            <div className="flex items-center gap-2">
